...
def compute_gp_difficulty_from_file(fname: str,
    ...
    这里采用简单直观的定义：
      d^{GP}_i = mean_{t,k} sigma_cal_i(t,k)
    """
    ...
    sigma = np.load(sigma_path)  # (T, K)
    d_gp = float(sigma.mean())   # 使用 mean 而不是 max
    return d_gp
...
class HeteroscedasticEFMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 限制 log_sigma2 的范围，防止 σ^2 太小/太大
        log_sigma2 = torch.clamp(log_sigma2, min=-4.0, max=4.0)

        sigma2 = torch.exp(log_sigma2) + 1e-6
        sigma2 = torch.clamp(sigma2, min=1e-3, max=1e3)
...
def compute_conformal_q_gp_scaled(alpha: float = 0.1,
    ...
    """
    GP-scaled σ-aware conformal：

      sigma_tot^2 = sigma_EF^2 + (beta * d^{GP})^2
    ...
    with torch.no_grad():
        for feats, ef, fnames in val_loader:
            ...
            sigma_tot = torch.sqrt(sigma_ef ** 2 + (beta * d_gp) ** 2 + 1e-6)
            residual = torch.abs(ef - mu)
            z = residual / (sigma_tot + 1e-6)
    ...
    print(f"[GP-scaled conformal] (VAL) alpha={alpha}, beta={beta}, q={q:.4f}")

    save_path = CHECKPOINT_DIR / "conformal_q_gp_scaled.npy"
    np.save(save_path, np.array([q, beta], dtype=np.float32))
    print(f"[GP-scaled conformal] Saved q & beta to {save_path}")

    return q


def compute_conformal_q_gp_binned(alpha: float = 0.1,
                                  num_bins: int = 5,
                                  batch_size: int = 64,
                                  checkpoint_path: Path = CHECKPOINT_DIR / "hetero_mlp_best.pt"):
    """
