...
class HeteroscedasticEFMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        raw_sigma = torch.clamp(raw_sigma, min=-10.0, max=10.0)  # 防止极端值
        sigma = torch.nn.functional.softplus(raw_sigma) + 1e-3    # >0
...
def heteroscedastic_gaussian_nll(y_true: torch.Tensor,
    ...
    nll = 0.5 * (((y_true - mu) ** 2) / var + torch.log(var + 1e-6))
    return nll
...
def compute_conformal_q_gp_scaled(alpha: float = 0.1,
    ...
    # 1) 先写一个小工具：收集某个 split 的 residual/sigma/d_gp
    def _collect_split(split_name: str):
        dataset = EchonetFeatureDataset(
            csv_path=CSV_PATH, gp_root=GP_ROOT,
            split_names=(SPLIT_VALUES[split_name],),
            use_calibrated_sigma=True,
        )
        loader = DataLoader(dataset, batch_size=batch_size,
                            shuffle=False, num_workers=4, pin_memory=True)

        all_residual, all_sigma, all_dgp = [], [], []
        with torch.no_grad():
            for feats, ef, fnames in loader:
                feats, ef = feats.to(device), ef.to(device)
                feats_norm = (feats - feat_mean) / feat_std
                mu, sigma_ef = model(feats_norm)
                d_gp = compute_gp_difficulty_batch(list(fnames), gp_root=GP_ROOT, device=device)
                residual = torch.abs(ef - mu)

                all_residual.append(residual.cpu().numpy())
                all_sigma.append(sigma_ef.cpu().numpy())
                all_dgp.append(d_gp.cpu().numpy())

        res = np.concatenate(all_residual)
        sig = np.concatenate(all_sigma)
        dgp = np.concatenate(all_dgp)

        mask = np.isfinite(dgp)   # 或 (dgp >= 0) 视你的定义
        return res[mask], sig[mask], dgp[mask]


    # 2) 收集 VAL / TEST
    res_val, sig_val, d_val = _collect_split("val")
    res_test, sig_test, d_test = _collect_split("test")

    # 3) 用 VAL 的 d_mean/d_std 标准化（保证校准/测试一致）
    d_mean = float(d_val.mean())
    d_std  = float(d_val.std() + 1e-6)
    d_norm_val  = np.maximum(0.0, (d_val  - d_mean) / d_std)
    d_norm_test = np.maximum(0.0, (d_test - d_mean) / d_std)

    beta_list, q_list = [], []
    val_cov_list, val_w_list = [], []
    test_cov_list, test_w_list = [], []

    for beta in beta_grid:
        # --- VAL：q 的标定仍在 VAL 上完成（正确的 conformal 流程） ---
        sigma_tot_val = np.sqrt(sig_val**2 + (beta*d_norm_val)**2 + 1e-6)
        z_val = res_val / (sigma_tot_val + 1e-6)
        q_beta = float(np.quantile(z_val, 1.0 - alpha))

        covered_val = res_val <= q_beta * sigma_tot_val
        val_cov = float(covered_val.mean())
        val_w   = float((2.0 * q_beta * sigma_tot_val).mean())

        # --- TEST：用同一个 q_beta 评估泛化覆盖率/宽度（这才是 κ 敏感性） ---
        sigma_tot_test = np.sqrt(sig_test**2 + (beta*d_norm_test)**2 + 1e-6)
        covered_test = res_test <= q_beta * sigma_tot_test
        test_cov = float(covered_test.mean())
        test_w   = float((2.0 * q_beta * sigma_tot_test).mean())

        beta_list.append(beta); q_list.append(q_beta)
        val_cov_list.append(val_cov); val_w_list.append(val_w)
        test_cov_list.append(test_cov); test_w_list.append(test_w)

    # 4) for 循环结束后再保存一次 CSV
    import pandas as pd
    sweep_df = pd.DataFrame({
        "kappa": beta_list, "q": q_list,
        "val_cov": val_cov_list, "val_width": val_w_list,
        "test_cov": test_cov_list, "test_width": test_w_list,
    })
    ...
    cov_arr = np.array(val_cov_list, dtype=np.float64)
    width_arr = np.array(val_w_list, dtype=np.float64)
    ...
    if np.any(ok_mask):
        # 在 coverage ≥ target - tol 的 β 中选 mean_width 最小
        feasible_idx = np.where(ok_mask)[0]
        best_idx = feasible_idx[np.argmin(width_arr[feasible_idx])]
        print(f"[GP-scaled] Choose beta from feasible set (coverage >= {target_coverage - cov_tolerance:.3f})")
    else:
        # 否则选 coverage 最接近 target 的 β
        best_idx = int(np.argmin(np.abs(cov_arr - target_coverage)))
        print(f"[GP-scaled] No beta reaches target; choose closest to {target_coverage:.3f}")
    ...
    print(f"[GP-scaled] Final choice on VAL: beta_best={beta_best:.3f}, q_best={q_best:.4f}, "
    ...
    np.save(save_path, np.array([q_best, beta_best, d_mean, d_std], dtype=np.float32))
